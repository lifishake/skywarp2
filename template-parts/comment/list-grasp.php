 ...
 * 来源: 破袜子原创
 */
function skywarp2_get_grasp_list()
{
    if ( !is_single() ) {
        return;
    }
    ...
    <?php
        foreach ($senselesses as $comment ) { ?>
            <li id="comment-<?php echo $comment->ID; ?>" <?php comment_class( 'grasp' ); ?>>
                <div class="comment-grasp vcard">
                    <?php echo '<a class="x7" href="'.$comment->author_url.'" rel="external nofollow" >'. get_avatar( $comment->author_email(), $args['avatar_size']).'</a>' ; ?>
                </div>
            </li>
        <?php }?>
    ?>
